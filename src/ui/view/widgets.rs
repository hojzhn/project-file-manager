...
pub fn file_row(file: &ProjectFileView) -> Element<'static, Message> {
    let mut r = row![
        ...
        text(format!("{} bytes", file.size_bytes)).size(12),
        text(file.modified_at.format("%Y-%m-%d").to_string()).size(12),
        button("Open").on_press(Message::OpenFileClicked(file.abs_path.clone())),
        button("Reveal").on_press(Message::RevealFileClicked(file.abs_path.clone())),
    ]
    .spacing(8.0)
    .align_y(Alignment::Center);

    r = match file.source {
        FileSource::Home => r.push(text("home").size(12)),
        FileSource::Child => {
            r = r.push(text("not moved").size(12));
            match file.child_file_id {
                Some(child_file_id) => r.push(button("Move").on_press(Message::MoveChildFileClicked(child_file_id))),
                None => r,
            }
        }
    };
...
pub fn iteration_view(state: &State, iteration: &Iteration) -> Element<'static, Message> {
    ...
    container(row![thumb, files_col].spacing(8.0)).padding(8.0).width(Length::Fill).into()
}
...
pub fn image_group_view(state: &State, group: &ImageGroup) -> Element<'static, Message> {
    ...
    let info = column![
        text(group.image.file_name.clone()).size(16),
        text(group.image.modified_at.format("%Y-%m-%d").to_string()).size(12),
        button("Open").on_press(Message::OpenFileClicked(group.image.abs_path.clone())),
    ]
    .spacing(4.0);
    ...
    container(col).padding(8.0).width(Length::Fill).into()
}
