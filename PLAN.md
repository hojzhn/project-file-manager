...
**Iteration-output matching** (`scanner::scan_relevant_directories`, `queries::rematch_child_files`):
...
- Scanning the relevant directories computes the same stripped base name for each configured child-extension file found there and links it to whichever project has a `home_files` row sharing that base name (`child_files.matched_project_id`, via `rematch_child_files`). This means every parent file in a project gets its own family of iterations matched, not just the first/original one. All configured relevant directories are scanned together in one pass before reconciling, so files from an already-scanned directory don't get spuriously marked missing by a later directory's scan.
- **Matched files are never moved automatically.** They stay physically in their relevant directory and just show up in the project's unified file view, tagged as unmoved, with a per-file "Move" button and a "Move all matched" button (`organize::move_child_file_into_project`). This was a deliberate reversal after initially building it as an automatic move.
- Adding a second parent file to an existing project currently only works by copying it into the project's folder externally and hitting "Rescan" (which also re-runs `rematch_child_files`) -- there's no in-app "add file to project" action yet.

**Two source tables instead of one**: `home_files` (physically inside a project's own folder, one folder per project, no overlap possible) and `child_files` (physically inside the configured relevant directories, linked to a project by filename match rather than by location -- called `rip_files` until the terminology was generalized, renamed via migration `SCHEMA_V4`). `queries::files_for_project` merges both into `ProjectFileView` rows tagged with `FileSource::Home` or `FileSource::Child` for display. This split is what makes the matching model work: a project's own folder is scanned normally, while the relevant directories are scanned once, globally, and fanned out to whichever projects match.
...
src/
  ...
  app.rs         - eframe UI: setup dialog, new-project flow, project view (current UI, being replaced by src/ui/)
  ui/            - iced migration scaffold: mod.rs, state.rs, message.rs, update.rs, subscription.rs,
                   thumbnails.rs, view/{mod, sidebar, setup_dialog, new_project_menu, scratch_dialog,
                   project_view, widgets}.rs -- all empty stubs so far, not yet wired up or referenced
  grouping.rs    - Iteration/ImageGroup/group_files: framework-agnostic grouping of files into
                   per-parent-file iteration families (moved out of app.rs so it's usable from either UI)
  model.rs       - Project, HomeFile, ChildFile, Settings, ExtensionRules, ProjectFileView, FileKind
  ...
  organize.rs    - move_child_file_into_project, collision-safe destination naming
...
**Planned UI migration**: moving from `eframe`/`egui` (immediate-mode) to `iced` (Elm-architecture: state + `Message` + `update` + `view`). `app.rs` is currently the *only* file with any UI-framework dependency -- everything else is already framework-agnostic, and `commands.rs`'s `Command`/`UiEvent` enum is already the clean boundary between the worker and whatever UI sits on top. `src/ui/` holds the empty scaffold for the new iced code; `app.rs` gets deleted once the migration is complete. `Command`/`UiEvent` (worker protocol) and the new `Message` enum (iced-internal) are meant to stay distinct -- `ui/update.rs` is the translation layer between them.
...
- The project view's scroll area has a known bug; low priority since the UI is being migrated to `iced` anyway (see "Planned UI migration" above), which may make the current egui-specific fix moot.
- The iced UI itself -- `src/ui/` is currently just an empty module scaffold.

## Verification

- `cargo test` covers: base-name suffix stripping, files matching by filename into a project's unified view without being moved, an explicit move relocating a file and flipping its source tag, unrelated files staying unmatched, iteration output for a second parent file in the same project getting matched too (not just the first/seed file), settings round-tripping a multi-entry relevant-directories list and custom extension rules, output split across two separately configured relevant directories both getting matched, a fully custom (non-default) parent/child extension relationship matching only the configured extensions, and `grouping::group_files` grouping iterations under the right parent file / leaving unmatched iterations unassigned.
