...
#[test]
fn rip_output_for_every_image_in_a_project_gets_matched_not_just_the_seed() {
    let root = tempdir("multi_root");
    let rip_dir = tempdir("multi_rip");
    ...
    fs::write(rip_dir.join("front-0.prt"), "job").unwrap();
    fs::write(rip_dir.join("back-0.prt"), "job").unwrap();
    scanner::scan_relevant_directories(&mut db.conn, &[rip_dir.clone()], &ExtensionRules::default()).unwrap();

    let files = queries::files_for_project(&db.conn, project.id).unwrap();
    assert!(
        files.iter().any(|f| f.file_name == "front-0.prt"),
        "RIP output for the first image must be matched"
    );
    assert!(
        files.iter().any(|f| f.file_name == "back-0.prt"),
        "RIP output for the second image must be matched too, not just the first/seed image"
    );
}
